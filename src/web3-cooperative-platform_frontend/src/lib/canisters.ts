...
export const getBackendActor = async (): Promise<_SERVICE | null> => {
  ...
  const agent = await HttpAgent.create({ host: DEFAULT_HOST, identity });
...
export const getAnonymousBackendActor = async (): Promise<_SERVICE> => {
  const agent = await HttpAgent.create({ host: DEFAULT_HOST });
